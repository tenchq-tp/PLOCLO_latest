...
async function fetchStudents() {
  ...
  const handleStudentUploadButtonClick = async () => {
    if (studentExcelData && studentExcelData.length > 0) {
      ...
      if (!window.confirm(`คุณต้องการอัปโหลดข้อมูลนักศึกษาจำนวน ${studentExcelData.length} รายการใช่หรือไม่?`)) {
        return;
      }
        ...
        <div
          ...
          <div
            ...
            <div className="student-management-container">
              ...
              {studentTypeError && (
                <div className="alert alert-danger mb-3">{studentTypeError}</div>
              )}
                ...
                <>
                  ...
                  {selectedYear !== "all" && !students.length && (
                    <div className="alert alert-info mt-4" style={{ textAlign: "center" }}>
                      {!students.length ? (
                        <>
                          No student data found for the academic year {selectedYear}.<br />
        ...
        {showEditStudentModal && (
          <div
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">Edit Student (ปี {selectedYear})</h5>
                  <button
                    type="button"
                    className="btn-close"
                    aria-label="Close"
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    {studentExcelData.length > 0 ? (
                      <>
                        <p>{t("Found")} {studentExcelData.length} {t("student records from Excel file.")}</p>
                        <div style={{ maxHeight: "300px", overflowY: "auto" }}>
                          <table className="table table-bordered table-striped">
                            ...
                            <tbody>
                              {studentExcelData.map((item, index) => (
                                <tr key={index}>
                                  <td>{item.student_id || item["รหัสนักศึกษา"] || "-"}</td>
                                  <td>{item.first_name || item["ชื่อ"] || "-"}</td>
                                  <td>{item.last_name || item["นามสกุล"] || "-"}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
